...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        val scope = CoroutineScope(Job() + Dispatchers.Main)
        scope.launch {
            var resultSpasavanja = spasiBiljke(this@MainActivity)
            when(resultSpasavanja) {
                is String -> {
                    var tempBiljke = ucitajBiljke(this@MainActivity)
                    when (tempBiljke) {
                        is List<Biljka> -> {
                            biljke = ArrayList<Biljka>(tempBiljke)
                            ...
                            biljkeRecyclerView.layoutManager =
                                ...
                                override fun onClick(v: View?) {
                                    if (lastFocus == "Medicinski") {
                                        ...
                                        scope.launch {
                                            ...
                                            when (tempBiljke) {
                                                is List<Biljka> -> {
                                                    biljke = ArrayList(tempBiljke)
                                                    medicinskiAdapter =
                                                        MedicinskiAdapter(biljke) { trenutnaBiljka ->
                                                            medicinskiAdapter.filtriraj(
                                                                trenutnaBiljka
                                                            )
                                                        }
                                                    biljkeRecyclerView.adapter = medicinskiAdapter
                                        ...
                                        scope.launch {
                                            ...
                                            when (tempBiljke) {
                                                is List<Biljka> -> {
                                                    biljke = ArrayList(tempBiljke)
                                                    kuharskiAdapter =
                                                        KuharskiAdapter(biljke) { trenutnaBiljka ->
                            ...
                            novaBiljkaButton.setOnClickListener {
                                ...
                                val intent =
                                    Intent(this@MainActivity, NovaBiljkaActivity::class.java)
                            ...
                            brzaPretragaButton.setOnClickListener {
                                ...
                                scope.launch {
                                    ...
                                    if (!boja.equals("") && !tekstPretrage.equals("")) {
                                        val noveBiljkeZaPrikazati =
                                            TrefleDAO(this@MainActivity).getPlantsWithFlowerColor(
                                                boja,
                                                tekstPretrage
                                            )
                                        val noOpClickListener: (Biljka) -> Unit = {}
                                        botanickiAdapter = BotanickiAdapter(
                                            noveBiljkeZaPrikazati,
                                            true,
                                            noOpClickListener
                                        )
                                        botanickiAdapter
                                    ...
                                    when (position) {
                                        0 -> {
                                            if (lastFocus == "Kuharski") {
                                                biljke =
                                                    kuharskiAdapter.trenutneBiljke as ArrayList<Biljka>
                                            } else if (lastFocus == "Botanički") {
                                                pretragaText.visibility = View.GONE
                                                brzaPretragaButton.visibility = View.GONE
                                                bojaSpinner.visibility = View.GONE
                                                if (pretragaIzvrsena) {
                                                    ...
                                                    biljke =
                                                        botanickiAdapter.trenutneBiljke as ArrayList<Biljka>
                                                }
                                            }
                                            medicinskiAdapter =
                                                MedicinskiAdapter(biljke) { trenutnaBiljka ->
