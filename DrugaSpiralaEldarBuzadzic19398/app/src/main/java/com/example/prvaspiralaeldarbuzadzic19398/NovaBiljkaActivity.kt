...
class NovaBiljkaActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val zemljisniTipStavke: Array<Zemljište> = Zemljište.values()
        ...
        dodajBiljku.setOnClickListener{
            ...
            if (nazivT.length < 2 || nazivT.length > 20){
                naziv.setError("Dužina teksta mora biti u segmentu [2,20]")
            ...
            if (!detektovanError){
                ...
                var selectedZT: ArrayList<Zemljište> = ArrayList()
                for (i in 0 until zemljisniTip.count){
                    if (selectedItemsZT.get(i)){
                        selectedZT.add(Zemljište.entries[i])
                    }
                }
                ...
                val novaDodanaBiljka = Biljka(nazivT, porodicaT, medicinskoUpozorenjeT, selectedMK, selectedPO, listaJela, selectedKT, selectedZT)
